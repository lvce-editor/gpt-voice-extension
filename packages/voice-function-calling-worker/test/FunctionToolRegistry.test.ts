...
test('returns registered function tool definitions', () => {
  expect(getRegisteredTools()).toEqual([
    ...
    expect.objectContaining({ name: 'expand_explorer_folder' }),
    expect.objectContaining({ name: 'collapse_explorer_folder' }),
    expect.objectContaining({ name: 'start_explorer_rename' }),
    expect.objectContaining({ name: 'open_explorer_context_menu' }),
