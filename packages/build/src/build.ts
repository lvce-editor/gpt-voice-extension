...
const voiceWorkWorker = path.join(root, 'packages', 'voice-work-worker')
...
await Promise.all([
  ...
  buildBundle(
    join(voiceWorkWorker, 'src', 'voiceWorkWorkerMain.ts'),
    join(root, 'dist', 'dist', 'voiceWorkWorkerMain.js'),
  ),
  esbuildBuild({
    banner: {
