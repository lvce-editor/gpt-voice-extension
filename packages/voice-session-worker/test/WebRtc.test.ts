...
test('createSessionConfig - includes registered function tools', () => {
  ...
  expect(config.session.instructions).toContain(
    'If a tool result contains a hint, follow it to retry or explain the problem.',
  )
  expect(config.session.instructions).toContain(
    'When the user asks to open a bare project-like name and it is unclear whether they mean a file or workspace, call get_recently_opened_folders first.',
  )
})
